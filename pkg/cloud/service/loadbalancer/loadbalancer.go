...
import (
	...
	capostrings "github.com/easystack/cluster-api-provider-openstack/pkg/utils/strings"
)

const (
	networkPrefix               string = "easystack-service"
...
func (s *Service) ReconcileLoadBalancer(openStackCluster *infrav1.OpenStackCluster, plan *ecnsv1.Plan, lbName string, Port []int) error {
	loadBalancerName := getLoadBalancerName(lbName)
	...
	lb, err := s.getOrCreateLoadBalancer(openStackCluster, loadBalancerName, openStackCluster.Status.Network.Subnet.ID, lbName, fixedIPAddress, lbProvider)
	if err != nil {
		return err
	}
	...
	if fip != nil {
		lbFloatingIP = fip.FloatingIP
	}
	if !openStackCluster.Spec.DisableAPIServerFloatingIP && fip == nil {
		var floatingIPAddress string
		fp, err := s.networkingService.GetOrCreateFloatingIP(openStackCluster, openStackCluster, lbName, floatingIPAddress)
		if err != nil {
			return err
		}
	...
	if openstack.IsOctaviaFeatureSupported(octaviaVersion, openstack.OctaviaFeatureVIPACL, lbProvider) {
...
func (s *Service) ReconcileLoadBalancerMember(openStackCluster *infrav1.OpenStackCluster, machine *clusterv1.Machine, openStackMachine *infrav1.OpenStackMachine, clusterName, ip string, port []int) error {
	if openStackCluster.Status.Network == nil {
...
func (s *Service) DeleteLoadBalancer(plan *ecnsv1.Plan, lbName string) error {
	loadBalancerName := getLoadBalancerName(lbName)
	lb, err := s.checkIfLbExists(loadBalancerName)
	if err != nil {
		return err
	...
	if lb.VipPortID != "" {
		...
		if fip != nil && fip.FloatingIP != "" {
			if err = s.networkingService.DisassociateFloatingIP(plan, fip.FloatingIP); err != nil {
				return err
			}
			if err = s.networkingService.DeleteFloatingIP(plan, fip.FloatingIP); err != nil {
				return err
			}
		}
	}
	...
	if err != nil && !capoerrors.IsNotFound(err) {
		record.Warnf(plan, "FailedDeleteLoadBalancer", "Failed to delete load balancer %s with id %s: %v", lb.Name, lb.ID, err)
		return err
	}

	record.Eventf(plan, "SuccessfulDeleteLoadBalancer", "Deleted load balancer %s with id %s", lb.Name, lb.ID)
	return nil
}
